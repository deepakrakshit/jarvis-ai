...
import type {
  ...
  JARVISPluginNodeHostCommandContext,
...
const COMPUTER_RESOURCE_HANDLE_PATTERN =
  "^jarvis:computer-resource:v1:[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$";
...
export type ComputerUseProvider = {
  ...
  isAvailable(): boolean;
  prepare?: (context: JARVISPluginNodeHostCommandAvailabilityContext) => Promise<void> | void;
  watchAvailability?: (
    context: JARVISPluginNodeHostCommandAvailabilityContext,
    onChange: () => void,
  ) => (() => void) | void;
...
type ComputerUseRegistrationApi = {
  registerNodeHostCommand(command: JARVISPluginNodeHostCommand): void;
};
...
export function registerComputerUseProvider(
  ...
  const getExecution = async (
    paramsJSON: string | null | undefined,
    context?: JARVISPluginNodeHostCommandContext,
  ) => {

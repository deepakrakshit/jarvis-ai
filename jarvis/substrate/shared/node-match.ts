...
export type NodeMatchCandidate = {
  ...
  connected?: boolean;
  /** Client id used to prefer current JARVIS nodes over legacy migration ties. */
  clientId?: string;
};
...
function isCurrentJARVISClient(clientId: string | undefined): boolean {
  const normalized = normalizeOptionalLowercaseString(clientId) ?? "";
  return normalized.startsWith("jarvis-");
}
...
function pickPreferredLegacyMigrationMatch(
  ...
  const current = matches.filter((match) => isCurrentJARVISClient(match.clientId));
  if (current.length !== 1) {
    return undefined;
  }
  ...
  // During Clawdbot -> JARVIS migration, a unique current client should win only

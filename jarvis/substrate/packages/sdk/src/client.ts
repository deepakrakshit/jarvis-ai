...
// High-level JARVIS SDK client. Namespaces below translate friendly SDK calls
...
/** Connection and transport options for the JARVIS SDK client. */
export type JARVISOptions = {
  gateway?: "auto" | (string & {});
  url?: string;
  token?: string;
  password?: string;
...
function resolveGatewayUrl(options: JARVISOptions): string | undefined {
  if (options.url) {
    return options.url;
  }
...
function assertNoUnsupportedRunOptions(params: AgentRunParams): void {
  ...
  throw new Error(
    `JARVIS Gateway does not support per-run SDK option${
...
function unsupportedGatewayApi(api: string): never {
  throw new Error(`${api} is not supported by the current JARVIS Gateway yet`);
}
...
type RunTerminalSource = { kind: "canonical" } | { kind: "chat"; eventType: JARVISEvent["type"] };
...
function readChatProjection(event: JARVISEvent): ChatProjection | undefined {
  const raw = event.raw;
...
function isAssistantRunEvent(event: JARVISEvent): boolean {
  return event.type === "assistant.delta" || event.type === "assistant.message";
}

function isTerminalRunEvent(event: JARVISEvent): boolean {
  return (
...
export class JARVIS {
  ...
  private readonly transport: JARVISTransport;
  private readonly normalizedEvents = new EventHub<JARVISEvent>();
  private readonly replayByRunId = new Map<string, JARVISEvent[]>();
  ...
  events(filter?: (event: JARVISEvent) => boolean): AsyncIterable<JARVISEvent> {
    return this.iterateEvents(filter);
  }

  runEvents(
    runId: string,
    filter?: (event: JARVISEvent) => boolean,
  ...
  private assertOpen(): void {
    if (this.closed) {
      throw new Error("JARVIS SDK client is closed");
    }
  }
  ...
  private async *iterateRunEvents(
    ...
    const toRunStreamEvent = (event: JARVISEvent): JARVISEvent | undefined => {
    ...
    const matches = (event: JARVISEvent) => event.runId === runId;
  ...
  private recordReplayEvent(event: JARVISEvent): void {
    if (!event.runId) {
      return;
    }
  ...
  private replaySnapshot(runId: string): JARVISEvent[] {
...
export class Agent {
  constructor(
    private readonly client: JARVIS,
    readonly id: string,
  ) {}
...
export class AgentsNamespace {
  constructor(private readonly client: JARVIS) {}
...
export class RunsNamespace {
  ...
  events(runId: string): AsyncIterable<JARVISEvent> {
...
class RpcNamespace {
  constructor(
    protected readonly client: JARVIS,

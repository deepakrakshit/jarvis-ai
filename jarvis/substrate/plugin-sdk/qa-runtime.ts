...
export async function closeQaRuntimeStores(tempRoot: string): Promise<void> {
  const [auth, { closeJARVISAgentDatabasesAsync }, state, paths] = await Promise.all([
    import("../agents/auth-profiles/sqlite.js"),
    import("../state/jarvis-agent-db.js"),
    import("../state/jarvis-state-db.js"),
    import("../state/jarvis-state-db.paths.js"),
  ]);
  ...
  await closeJARVISAgentDatabasesAsync(tempRoot);
  await state.closeJARVISStateDatabaseByPathAsync(
    paths.resolveJARVISStateSqlitePath({ JARVIS_STATE_DIR: path.join(tempRoot, "state") }),
  );
}

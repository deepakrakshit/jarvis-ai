...
import { normalizeCsvOrLooseStringList } from "@jarvis/normalization-core/string-normalization";
...
/** Parses the JSON5 JARVIS manifest block embedded inside a string frontmatter field. */
export function resolveJarvisManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
/** Extracts normalized runtime requirement lists from an JARVIS manifest block. */
export function resolveJarvisManifestRequires(
  metadataObj: Record<string, unknown>,
): JarvisManifestRequires | undefined {
  const requiresRaw =
...
export function resolveJarvisManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
/** Extracts normalized OS allowlist entries from an JARVIS manifest block. */
export function resolveJarvisManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

type ParsedJarvisManifestInstallBase = {
...
export function parseJarvisManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedJarvisManifestInstallBase | undefined {
  ...
  const spec: ParsedJarvisManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyJarvisManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedJarvisManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {

/**
 * JARVIS WhatsApp VoIP Engine - Signaling Bridge.
 *
 * Encrypts and manages VoIP offer/answer stanzas, TC tokens, and routing
 * across WhatsApp devices.
 */
...
export type SignalingBridgeConfig = {
  sock: BaileysSocket;
  onCallTerminated?: (callId: string, reason: string) => void;
};
...
export class SignalingBridge {
  ...
  readonly #onCallTerminated?: (callId: string, reason: string) => void;

  constructor(config: SignalingBridgeConfig) {
    this.#sock = config.sock;
    this.#onCallTerminated = config.onCallTerminated;
  }
  ...
  resolveLid = async (pnJid: string): Promise<string | undefined> => {
    ...
    if (!lid) {
      try {
        const cleanNumber = pnJid.split("@")[0];
        if (typeof (this.#sock as any).onWhatsApp === "function") {
          await (this.#sock as any).onWhatsApp(cleanNumber);
        }
    ...
    switch (usableNode.tag) {
      ...
      default:
        ...
        if (callIdForRouting && (usableNode.tag === "terminate" || usableNode.tag === "reject")) {
          ...
          const reason = usableNode.tag === "reject" ? "rejected" : "remote_ended";
          this.#onCallTerminated?.(callIdForRouting, reason);
        }
        break;
    }
  };

/**
 * JARVIS WhatsApp VoIP Engine - WebAssembly Runtime.
 *
 * Runs the WhatsApp Web VoIP WebAssembly stack inside a Node.js VM context
 * with worker thread pthread pooling.
 */
...
export class WasmEngine {
  ...
  constructor(config: WasmEngineConfig = {}) {
    const findDefaultBasePath = (): string => {
      const candidates = [
        process.cwd(),
      ...
      for (const cand of candidates) {
        if (fs.existsSync(path.join(cand, "assets", "wasm", "whatsapp.wasm"))) {
          return cand;
        }
      }
      return process.cwd();
